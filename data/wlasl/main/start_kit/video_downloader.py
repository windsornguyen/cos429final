    ...
    with ThreadPoolExecutor(max_workers=16) as executor:
        ...
        for entry in content:
            ...
            for inst in instances:
                ...
                future = executor.submit(download_method, video_url, saveto, video_id)
                futures.append(future)

        for _ in tqdm(as_completed(futures), total=len(futures), desc='Downloading non-YouTube videos'):
            pass
...
def download_yt_videos(indexfile, saveto='raw_videos'):
    ...
    for entry in tqdm(content, desc='Downloading YouTube videos'):
        gloss = entry['gloss']
        instances = entry['instances']
